...
def test_post_simulacao_success():
    ...
    """
    payload = {"valor_imovel": 500000.0, "percentual_entrada": 10.0, "anos_contrato": 3}
...
def test_post_simulacao_invalid_percentual_entrada():
    ...
    """
    payload = {"valor_imovel": 100000.0, "percentual_entrada": 4.0, "anos_contrato": 2}  # Inválido (menor que 5)
    response = client.post("/simulacao", json=payload)
...
def test_post_simulacao_invalid_anos_contrato():
    ...
    """
    payload = {"valor_imovel": 100000.0, "percentual_entrada": 10.0, "anos_contrato": 0}  # Inválido (menor que 1)
    response = client.post("/simulacao", json=payload)
...
def test_post_simulacao_invalid_json_format():
    ...
    payload = {"valor_imovel": "abc", "percentual_entrada": 10.0, "anos_contrato": 2}  # Inválido
    response = client.post("/simulacao", json=payload)

    assert response.status_code == 422
    assert "detail" in response.json()
    assert "Input should be a valid number, unable to parse string as a number" in str(
        response.json()
    )  # Pydantic reporta erro de tipo
